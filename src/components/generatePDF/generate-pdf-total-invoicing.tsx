...
import { MetricsTotalInvoicingProps } from '../../app/(panel)/dashboard/reports/types/invoicing';

interface GeneratePDFProps {
  data: MetricsTotalInvoicingProps;
}

export default function GeneratePDFTotalInvoicing({ data }: GeneratePDFProps) {
  ...
  useEffect(() => {
    const loadLibs = async () => {
      ...
      const gerarPDF = () => {
        ...
        doc.setFontSize(14);
        doc.text("Receita Total", pageWidth / 2, yTitle, { align: 'center' });
        doc.setFontSize(10);
        doc.text("Desde o início da operação", pageWidth / 2, 55, { align: 'center' });
        ...
        doc.text(String(data.totalAppointments), 60, finalY);

        doc.setFont('helvetica', 'bold');
        doc.text("Faturamento Total:", 10, finalY + 10);
        doc.setFont('helvetica', 'normal');
        doc.text(`R$ ${data.totalInvoicing.toFixed(2).replace('.', ',')}`, 60, finalY + 10);
