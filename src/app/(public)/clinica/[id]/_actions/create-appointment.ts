"use server"

import prisma from "@/lib/prisma";
import { z } from "zod"

const formSchema = z.object({
  name: z.string().min(1, "O nome é obrigatório"),
  email: z.string().email("O email é obrigatório"),
  phone: z.string().min(1, "O telefone é obrigatório"),
  age: z.number().min(0).optional(),
  dateOfBirth: z.date().optional(),
  symptoms: z.string().optional(),
  secondary: z.string().optional(),
  complaints: z.string().optional(),
  useOfAnyMedication: z.boolean().optional(),
  bePregnant: z.boolean().optional(),
  eatingRoutine: z.boolean().optional(),
  physicalActivities: z.string().optional(),
  date: z.date(),
  serviceId: z.string().min(1, "O serviço é obrigatório"),
  time: z.string().min(1, "O horário é obrigatório"),
  clinicId: z.string().min(1, "O horário é obrigatório"),
});
type FormSchema = z.infer<typeof formSchema>;

export async function createNewAppointment(formData: FormSchema) {
  const schema = formSchema.safeParse(formData);

  if (!schema.success) {
    return {
      error: schema.error.issues[0].message
    }
  }
  try {
    const selectedDate = new Date(formData.date);


    const year = selectedDate.getFullYear();
    const month = selectedDate.getMonth();
    const day = selectedDate.getDate();

    const appointmentDate = new Date(Date.UTC(year, month, day, 0, 0, 0, 0));

    const newAppointment = await prisma.appointment.create({
      data: {
        name: formData.name,
        email: formData.email,
        phone: formData.phone,
        age: formData.age,
        dateOfBirth: formData.dateOfBirth,
        symptoms: formData.symptoms,
        secondary: formData.secondary,
        complaints: formData.complaints,
        useOfAnyMedication: formData.useOfAnyMedication,
        bePregnant: formData.bePregnant,
        eatingRoutine: formData.eatingRoutine,
        physicalActivities: formData.physicalActivities,
        time: formData.time,
        appointmentDate: appointmentDate,
        serviceId: formData.serviceId,
        userId: formData.clinicId
      }
    });

    return {
      data: newAppointment
    }
  } catch (error) {
    return {
      error: "Erro ao realizar agendamento"
    }
  }
}